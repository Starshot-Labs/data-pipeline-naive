...
P3-SAM has its own service because its PyTorch/CUDA/Sonata stack is unrelated to the scene
pipeline. Your local CUDA folders are not used: the image builds against CUDA 12.4 inside
Modal and compiles `P3-SAM/utils/chamfer3D` there.

```powershell
modal setup                  # only if this machine is not signed in yet
npm run deploy-p3sam         # builds and deploys dc-p3sam
npm run prefetch-p3sam       # optional: downloads both checkpoints before the first request
npm run dev                  # open http://localhost:5173/p3sam.html
```

`deploy-p3sam` is the slow command the first time: it clones the pinned upstream repository,
installs Torch 2.4/cu124 and compiles the Chamfer CUDA extension. `prefetch-p3sam` downloads
the 451 MB P3-SAM checkpoint and 434 MB Sonata checkpoint onto `p3sam-cache`; skipping it is
safe, but the first browser request then pays that download and model-load delay.

The deployed endpoint defaults to
`https://starshot-aitools--dc-p3sam-web.modal.run`. Set `P3SAM_BASE_URL` in `.env` if the
Modal workspace slug differs. Uploads return immediately, the local server polls the Modal
job, and completed artifacts land in `data/p3sam-results/`.

P3-SAM cleans the mesh before assigning labels, so its face indices refer to the returned
`parts.glb`, not necessarily the uploaded file. Each result contains that GLB, `labels.bin`
(little-endian `int32`, one label per returned face), and `result.json`.

The upstream P3-SAM code and weights use the
[Tencent Hunyuan 3D-Part Community License](https://github.com/Tencent-Hunyuan/Hunyuan3D-Part/blob/main/LICENSE),
which excludes use in the EU, UK, and South Korea. Sonata weights are CC BY-NC 4.0. Review
both before exposing this as anything beyond a private local tool.
...
is what keeps it away from the hand-placed exports in `data/dataset/`.
...
- `/viewer.html` — loads any `data/dataset/` sample's GLBs raw.
- `/place.html` — drop two GLBs in from anywhere on disk, each posed as its own file has it.
  Nothing has to be in `data/models/` or `data/dataset/` first, and nothing is sent to the
  server.
- `/` — the manual tool: load two GLBs, pose them by hand, export a sample.
- `/scene.html` — a different comparison entirely: one 3D-FRONT room out of `data/3d-front/`,
  one instruction, and a tile per LLM showing how it rearranged the room. Runs are made on the
  page and each model is asked from it. See the README's **Scene editing**.
- `/p3sam.html` — upload a GLB, OBJ, or PLY to P3-SAM on Modal, follow its live job stage, and
  inspect, hide, explode, or download the automatically segmented parts.

### Odds and ends

| Command                 | What it does                                                              |
| ----------------------- | ------------------------------------------------------------------------- |
| `npm run backfill`      | Adds missing bounding-box fields to old `data/dataset/` samples.          |
| `npm run audit-front3d` | Verifies every assembled room can be matched exactly to its object GLBs.  |
| `npm run clean-front3d`   | Normalizes assembled room IDs in place, with JSON-chunk backups.       |
| `npm run restore-front3d` | Restores original assembled-room JSON chunks from those backups.       |
| `npm run pull-scenes`     | Copies rooms off `3D-Front` into `data/3d-front/`.                      |
...
Everything this repo writes to disk sits under `data/`, one directory per kind:

```
data/
  models/                   source GLBs the manual tool loads
  dataset/                  hand-made samples from the manual tool
  generated/                the pipeline's samples — the small half of each
    <anchor>_<uuid>/
  ...
  placement-set/            the anchor, photo and phrase both experiments read
  placement-results/        test-placement.mjs — one folder per sample, per model
  edit-results/             test-edit.mjs
  segment-results/          PartField results from test-segment.mjs and segment.html
  p3sam-results/            P3-SAM results downloaded by p3sam.html
  archive/                  older runs, kept but read by nothing
  logs/                     captured service output
```

`data/placement-set/` is the only one in the repo; the rest is local. Each has its own
override — `MODELS_DIR`, `DATASET_DIR`, `GENERATED_DIR` and so on — and `DATA_DIR` moves the
whole tree at once. All of them are resolved in one place, `pipeline/paths.mjs`, which is also
what loads `.env`.
...
`.trellis-campaign.json` and `.trellis-stage/` are working files the mesh stage keeps at the
repo root. Both are gitignored, and both clean themselves up.
...
| `GENERATED_DIR`                                   | `./data/generated`                             | Where the local half of each sample gets written.    |
| `DATA_DIR`                                        | `./data`                                       | Move every directory at once, onto another disk.     |
...
| `P3SAM_BASE_URL`                                  | deployed `dc-p3sam` endpoint                   | Use another workspace or deployment.                 |
| `P3SAM_TIMEOUT_S` / `P3SAM_POLL_S`                | `3600` / `5`                                   | P3-SAM job deadline and polling interval.             |
| `P3SAM_RESULTS_DIR`                               | `./data/p3sam-results`                         | Where completed P3-SAM artifacts are cached locally. |
