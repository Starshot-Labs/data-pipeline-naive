 ...
 * world-space triangles as produced by `sceneTriangles`, `base` its lit shade,
 * and `dither` renders it screen-door transparent (every other subpixel), which
 * the supersampled downscale turns into a smooth ghost. That is what lets a
 * scene view show an object placed inside a cavity. The projection is
 * orthographic, so only the view direction matters, never a camera distance.
 */
export async function renderView(layers, { size = 1024, view = ISO_VIEW } = {}) {
  ...
  for (const { projected, base, dither } of projectedLayers) {
    for (let t = 0; t < projected.length; t += 9) {
      ...
      const shade = Math.min(255, Math.round(base * (AMBIENT + DIFFUSE * lambert)));
